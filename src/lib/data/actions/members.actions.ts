 ...
 *
 * @param uidToMakeAdmin - The user ID of the user to be promoted.
 ...
 */
export const makeUserAdminAction = withSession(
  async (uid: string, uidToMakeAdmin: string, class_id: string) => {
    const adminResult = await makeUserAdminInFirestore(
      uidToMakeAdmin,
      class_id
    );
    if (adminResult) {
